...
class Leaders extends CI_Controller {
	...
	function index()
  {
    ...
    $data['main'] = 'leaders';
    $data['filez'] = 'leaders';
    $data['functionz'] = 'leaders';
    $data['labelz'] = 'Leaders';
    $this->load->view('template', $data);
	}
	
	function leaders_new()
	{
		permission();
    $data = array();
    $data['filez'] = 'leaders';
    $data['functionz'] = 'leaders';
    $data['labelz'] = 'Leaders';
    $this->load->view('leaders_new', $data);
	}
	
	function leaders_list($trash=0)
  {
  	...
  	if(isset($_POST['order'][0]['column'])) $ord = $kolom[$_POST['order'][0]['column']]." ".$_POST['order'][0]['dir'];
		...
		$query_no_limit = "SELECT * FROM kg_leaders WHERE is_delete=0 AND ".$where." ORDER BY ".$ord;
		$query = "SELECT * FROM kg_leaders WHERE is_delete=0 AND ".$where." ORDER BY ".$ord." LIMIT ".$_POST['start'].",".$_POST['length'];
	  ...
	  foreach ($list->result_array() as $r) {
		  ...
		  $check = "<a href='".site_url('leaders/leaders_detail/'.$r['id'])."'><i class='icon icon-pencil'></i></a>";
  ...
  function leaders_detail($id=0)
	{
		permission();
    $data = GetHeaderFooter();
    $data['main'] = 'leaders_detail';
    $data['filez'] = 'leaders';
    $data['functionz'] = 'leaders';
    $data['labelz'] = 'Leaders';
    $data['id'] = $id;
    if($id) {
    	$q = GetAll("leaders", array("id"=> "where/".$id))->result_array()[0];
    	$data['val'] = $q;
    } else $data['val'] = array();
    $this->load->view('template', $data);
	}
	
	function leaders_aktif()
	{
		$idz = $this->input->post("idz");
		if($idz) {
			if($this->input->post("akt")==2) {
				$this->db->where("id", $idz);
				$this->db->update("leaders", array("is_delete"=> 1));
			}
		}
	}
  
  function leaders_add()
  {
  	ini_set('memory_limit', -1);
    $admin_id=permission();
    if($admin_id==1) {
	    ...
	    $post['slug'] = url_title($post['title']);
			...
			$cek_id = GetValue("id","leaders",array("id"=> "where/".$param['id']));
			if(!$cek_id) {
	    	...
	    	$post['create_user_id'] = $admin_id;
				...
				$post['modify_user_id'] = $admin_id;
				$post['modify_date'] = date("Y-m-d H:i:s");
				if($this->db->insert("leaders", $post)) $this->session->set_flashdata("message", "success-Add Success");
	    	else $this->session->set_flashdata("message", "danger-Add Failed");
	    } else {
	    	$record = GetAll('leaders',array('id'=>'where/'.$cek_id))->row_array();
				if (!$this->upload->do_upload('image')){
	    	...
	    	if($this->db->update("leaders", $post)) $this->session->set_flashdata("message", "success-Edit Success");
	    	else $this->session->set_flashdata("message", "danger-Edit Failed");
	    }
	  }
    
    redirect(site_url('leaders'));
  }
}
