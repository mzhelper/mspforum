...
class committee extends CI_Controller {
	...
	function index()
  {
    ...
    $data['main'] = 'committee';
    $data['filez'] = 'committee';
    $data['functionz'] = 'committee';
    $data['labelz'] = 'Organizing Committee';
    $this->load->view('template', $data);
	}
	
	function committee_new()
	{
		permission();
    $data = array();
    $data['filez'] = 'committee';
    $data['functionz'] = 'committee';
    $data['labelz'] = 'Organizing Committee';
    $this->load->view('committee_new', $data);
	}
	
	function committee_list($trash=0)
  {
  	...
  	if(isset($_POST['order'][0]['column'])) $ord = $kolom[$_POST['order'][0]['column']]." ".$_POST['order'][0]['dir'];
		...
		$query_no_limit = "SELECT * FROM kg_news WHERE is_delete=0 AND ".$where." ORDER BY ".$ord;
		$query = "SELECT * FROM kg_news WHERE is_delete=0 AND ".$where." ORDER BY ".$ord." LIMIT ".$_POST['start'].",".$_POST['length'];
	  ...
	  foreach ($list->result_array() as $r) {
		  ...
		  $check = "<a href='".site_url('committee/committee_detail/'.$r['id'])."'><i class='icon icon-pencil'></i></a>";
  ...
  function committee_detail($id=0)
	{
		permission();
    $data = GetHeaderFooter();
    $data['main'] = 'committee_detail';
    $data['filez'] = 'committee';
    $data['functionz'] = 'committee';
    $data['labelz'] = 'Organizing Committee';
    $data['id'] = $id;
