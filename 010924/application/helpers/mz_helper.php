...
if (!function_exists('permission')){
	...
	{
		$CI =& get_instance();
		if(!$CI->session->userdata("admin_grup")){
			redirect(site_url("login"));
			/*$CI->session->set_userdata('admin','Mazhters');
			$CI->session->set_userdata('admin_grup','8910');
			$CI->session->set_userdata('admin_id','1');*/
		}
...
if (!function_exists('GetUserGrup')){
	function GetUserGrup()
	{
		$CI =& get_instance();
		return $CI->session->userdata("admin_grup");
	}
}
...
if (!function_exists('cek_akses')){
	function cek_akses($db, $id_menu, $admin_grup)
	{
		$CI =& get_instance();
		$CI->db->where("id_user_grup", $admin_grup);
...
if (!function_exists('GetKabList')){	
	function GetKabList($id_prov=0, $id_kab=0)
	{
		$opt=array();
		$opt[''] = "- Pilih Kota -";
		$q = GetAll("kota", array("provinsi_id"=> "where/".$id_prov));
		foreach($q->result_array() as $r) {
			$opt[$r['id']] = $r['kabupaten'];
		}
		$xx = form_dropdown("id_city",$opt,isset($id_kab) ? $id_kab : "","class=\"form-control\"");
		return $xx;
	}
}
...
if (!function_exists('GetOptNewsCat')){	
	function GetOptNewsCat()
	{
		$opt = array();
		$q = GetAll("news_cat", array("is_delete"=> "where/0"));
		foreach($q->result_array() as $r)
		{
			$opt[$r['id']] = $r['title'];
		}
...
if (!function_exists('GetOptCalenderCat')){	
	function GetOptCalenderCat()
	{
		$opt = array();
		$q = GetAll("calender_cat", array("is_publish"=> "where/1", "is_delete"=> "where/0"));
		foreach($q->result_array() as $r)
		{
			$opt[$r['id']] = $r['title'];
		}
...
if (!function_exists('GetOptCalenderLokasi')){	
	function GetOptCalenderLokasi()
	{
		$opt = array();
		$q = GetAll("calender_lokasi", array("is_publish"=> "where/1", "is_delete"=> "where/0"));
		foreach($q->result_array() as $r)
		{
			$opt[$r['id']] = $r['title'];
		}
...
if (!function_exists('GetOptSpeakers')){	
	function GetOptSpeakers()
	{
		$opt = array();
		$q = GetAll("speakers", array("is_publish"=> "where/1", "title"=> "order/asc"));
		foreach($q->result_array() as $r)
		{
			$opt[$r['id']] = $r['title'];
		}
